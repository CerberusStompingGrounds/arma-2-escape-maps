...
drn_fnc_Escape_OnSpawnGeneralSoldierUnit = {
	...
	if (random 100 < 60) then {
		...
		_weaponar = drn_arr_Escape_Weapons select floor(random(count(drn_arr_Escape_Weapons)));
		_this addweapon (_weaponar select 0);
		for[{_i=0},{_i<(random(6)+2)},{_i=_i+1}] do {
			_this addmagazine ((_weaponar select 1) select 0);
		};
		if(count(_weaponar select 1)>1) then {
			for[{_c=0},{_c<(random(4)+4)},{_c=_c+1}] do {
				_mag = (((_weaponar select 1) select 1) select floor(random(count(((_weaponar select 1) select 1)))));
				hint format["%1",_mag];
				_this addmagazine _mag;
			};
		};
    };
	if (random 100 < 30) then {
       if(!(_this hasweapon "Makarov")) then {
		   _weaponar = drn_arr_Escape_Pistols select floor(random(count(drn_arr_Escape_Pistols)));
		   _this addweapon (_weaponar select 0);
			for[{_i=0},{_i<random(8)},{_i=_i+1}] do {
				_this addmagazine (_weaponar select 1);
			};
	   };
    };
...
drn_fnc_Escape_FindGoodPos = {
    ...
    for [{_i = 0},  {_i < 1}, {_i = _i + 1}] do {
        ...
        while {!_isOk} do {
            if (random 100 > 60) then {
                _startPos = + [2400 + random 2000, 2000 + random 2000]; // Most difficult place
            }
            else {
                if (random 100 > 50) then {
                    _startPos = + [2000 + random 2400, 3000 + random 1000]; // Difficult place
                }
                else {
                    _startPos = + [500 + random 3900, 500 + random 3500]; // Easiest place
                };
            };
