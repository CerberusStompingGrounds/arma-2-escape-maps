    ...
    [_volume, _showIntro, _showPlayerMapAndCompass, _isJipPlayer, _useRevive] spawn {
        ...
        if (_isJipPlayer) then {
            ...
            if (_showIntro) then {            
               ...
               ["<t size='0.9'>" + "Escape Aliabad" + "</t>",0.02,0.3,3,-1,0,3011] spawn bis_fnc_dynamicText;
                ["<t size='0.5'>" + "based on Escape Chernarus" + "</t>",0.02,0.4,3,-1,0,3012] spawn bis_fnc_dynamicText;
                ...
                0 cutText ["", "BLACK FADED"];
                ["Somewhere in Aliabad", str (date select 2) + "/" + str (date select 1) + "/" + str (date select 0) + " " + str (date select 3) + ":00"] spawn BIS_fnc_infoText;
            };
        };
