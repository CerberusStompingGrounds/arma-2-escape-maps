    ...
    [_volume, _showIntro, _showPlayerMapAndCompass, _isJipPlayer, _useRevive] spawn {
        ...
        if (_isJipPlayer) then {
            ...
            if (_showIntro) then {            
               ...
               ["<t size='0.9'>" + "Escape Podagorsk" + "</t>",0.02,0.3,3,-1,0,3011] spawn bis_fnc_dynamicText;
                ...
                0 cutText ["", "BLACK FADED"];
                ["Somewhere in Podagorsk", str (date select 2) + "/" + str (date select 1) + "/" + str (date select 0) + " " + str (date select 3) + ":00"] spawn BIS_fnc_infoText;
            };
        };
