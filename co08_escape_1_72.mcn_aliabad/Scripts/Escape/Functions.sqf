...
drn_fnc_Escape_OnSpawnGeneralSoldierUnit = {
	...
	if (random 100 < 30) then {
		...
		for[{_i=0},{_i<random(6)},{_i=_i+1}] do {
    ...
    for [{_i = 0},  {_i < 1}, {_i = _i + 1}] do {
        ...
        while {!_isOk} do {
            if (random 100 > 60) then {
                _startPos = + [3200 + random 1200, 3000 + random 1000]; // Most difficult place
            }
            else {
                if (random 100 > 50) then {
                    _startPos = + [2000 + random 2400, 2000 + random 2000]; // Difficult place
                }
                else {
